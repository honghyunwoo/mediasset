...
function getFormSubmitConfig() {
  ...
  const targetEmail = process.env.FORMSUBMIT_EMAIL || siteConfig.email;

  return {
    endpoint: `${FORMSUBMIT_BASE_URL}/${encodeURIComponent(targetEmail)}`,
    targetEmail,
  };
}
...
async function sendFormSubmitNotification(inquiry) {
  ...
  const response = await fetch(config.endpoint, {
    ...
    body: JSON.stringify({
      ...
      _subject: buildInquiryEmailSubject(inquiry),
      _template: "table",
    }),
  });
  ...
  if (!response.ok) {
    ...
    throw new Error(`FormSubmit 전송에 실패했습니다. ${errorText}`);
  }

  const result = await response.json();

  if (result.success === false) {
    throw new Error(`FormSubmit 전송에 실패했습니다. ${result.message || "알 수 없는 오류"}`);
  }
...
export async function sendInquiryNotification(inquiry) {
  const formSubmitResult = await sendFormSubmitNotification(inquiry);
